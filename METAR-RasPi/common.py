...
# ... after station_to_ident function

def check_internet_connection(host: str = "8.8.8.8", port: int = 53, timeout: int = 3) -> bool:
    """
    Pings a reliable host (Google's public DNS server) to check for internet connectivity.

    Args:
        host: The host to check (e.g., Google DNS).
        port: The port to use for the check.
