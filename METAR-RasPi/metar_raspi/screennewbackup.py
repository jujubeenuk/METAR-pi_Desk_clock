...
class METARScreen:
    ...
    def from_session(cls, session: dict, size: Coord) -> Self:
        ...
        station = session.get("station", "KIPJ")
    ...
    def __draw_info_elements(self, data: MetarData, units: Units) -> None:
    ...
    def draw_main(self) -> None:
        ...
        if not (self.metar.data and self.metar.units):
            ...
            return
        self.win.fill(self.c.WHITE)
        self.__draw_info_elements(self.metar.data, self.metar.units)
        self.buttons = [
            IconButton(
    ...
    def draw_info_screen(self) -> None:
        ...
        for text, key, font in (
            ...
            ("Fork by J.D. Mohrmann", "name", FONT_S3),
...
def main() -> None:
    ...
    # -----------------------------------------------------------------
    # ADD INTERNET CHECK HERE
    if not common.check_internet_connection():
        logger.critical("No internet connection found. Shutting down.")
        print("ERROR: No internet connection found. Shutting down.")
        sys.exit(1) # Exits the program with a non-zero status
    # -----------------------------------------------------------------
